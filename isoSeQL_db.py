...
def make_db(database):
	...
	c.execute("INSERT INTO versionInfo(name, visoSeQL) VALUES (?,?)", (database, 'v1.1.0'))
	c.execute("CREATE TABLE isoform (id INTEGER PRIMARY KEY, chr TEXT, strand TEXT, junctions TEXT, gene TEXT, iso_exons INTEGER, subcategory TEXT, canonical TEXT, IEJ INTEGER, category TEXT, UNIQUE(chr, strand, junctions, gene, category))")
	...
	c.execute("CREATE TABLE ends_counts (ends_id TEXT, exp INTEGER, read_count INTEGER, FOREIGN KEY(ends_id) REFERENCES isoform_ends(id), FOREIGN KEY(exp) REFERENCES exp(id), UNIQUE(ends_id, exp, read_count))")
