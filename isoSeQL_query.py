...
def make_bed(db, exp, outPrefix, name):
	...
	while e < len(exp_list):
		df_bed = pd.read_sql("SELECT DISTINCT i.id,i.chr,x.start,x.end,i.strand,i.category,i.iso_exons,i.junctions, x.ex_sizes, x.ex_starts FROM isoform i INNER JOIN isoform_ends x on i.id = x.isoform_id WHERE x.exp = ?", conn, params=(exp_list[e],))
